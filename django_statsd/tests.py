...
from django_statsd.patches import import_patches, utils
...
from django_statsd.views import _process_summaries, process_key
...
class TestIncr(DjangoTestCase):
    ...
    def test_graphite_response(self):
        gmw = middleware.GraphiteMiddleware(lambda x: x)
...
class TestTiming(TestCase):
    ...
    def test_request_timing(self):
        ...
        gmw = middleware.GraphiteRequestTimingMiddleware(lambda x: x)
        with patch('django_statsd.middleware.statsd') as statsd_mock:
            gmw.process_view(self.req, func, tuple(), dict())
            gmw.process_response(self.req, self.res)
        self.assertEqual(statsd_mock.timing.call_count, 3)
        names = [
            f'view.{func.__module__}.{func.__name__}.GET',
            f'view.{func.__module__}.GET',
            'view.GET',
        ]
    ...
    def test_request_timing_view_not_a_function(self):
        ...
        self.assertEqual(statsd_mock.timing.call_count, 3)
        names = [
            f'view.{func.__class__.__module__}.{func.__class__.__name__}.GET',
            f'view.{func.__class__.__module__}.GET',
            'view.GET',
        ]
        for expected, (args, kwargs) in zip(names, statsd_mock.timing.call_args_list):
            self.assertEqual(expected, args[0])
    ...
    def test_request_timing_tastypie(self):
        func = lambda x: x  # noqa: E731
        gmw = middleware.TastyPieRequestTimingMiddleware(lambda x: x)
        with patch('django_statsd.middleware.statsd') as statsd_mock:
            gmw.process_view(
                self.req,
                func,
                tuple(),
                {'api_name': 'my_api_name', 'resource_name': 'my_resource_name'},
            )
            gmw.process_response(self.req, self.res)
        self.assertEqual(statsd_mock.timing.call_count, 3)
        names = [
            'view.my_api_name.my_resource_name.GET',
...
class TestClient(TestCase):
    ...
    @log_capture()
    def test_log_client(self, captured_log):
        statsd = StatsdClientProxy()
        ...
        captured_log.check(
            ('statsd', 'INFO', 'Timing: testing.timing, 1, 1'),
    ...
    def test_refresh_client(self):
        ...
        sleep(1)
        statsd.incr('test')
        self.assertNotEqual(old_client, statsd._client)
...
class TestRecord(DjangoTestCase):
    ...
    def setUp(self):
        ...
        self.good = {'client': 'boomerang', 'nt_nav_st': 1, 'nt_domcomp': 3}
        self.stick = {
            'client': 'stick',
    ...
    def test_boomerang_almost(self):
        assert self.client.get(self.url, {'client': 'boomerang'}).status_code == 400

    def test_boomerang_minimum(self):
        content = self.client.get(
            self.url,
            {'client': 'boomerang', 'nt_nav_type': 'undefined', 'nt_nav_st': 1},
        ).content.decode()
    ...
    @patch.object(
        settings,
        'STATSD_RECORD_KEYS',
        ['unknown.key', 'window.performance.timing.navigationStart'],
    )
    def test_boomerang_unknown_key(self):
        content = self.client.get(self.url, {'client': 'boomerang', 'nt_nav_st': 1}).content.decode()
        assert content == 'recorded'
...
class TestViewFunctions(DjangoTestCase):
    ...
    def test_process_key_navigation_type(self):
        ...
        names = [
            'window.performance.navigation.type.navigate',
    ...
    def test_process_key_redirect_count(self):
        ...
        self.assertEqual(
            statsd_mock.incr.call_args_list[0][0][0],
        ...
        for operation, query in list(self.example_queries.items()):
            with patch('django_statsd.patches.db.statsd') as statsd_mock:
                ...
                self.assertEqual(
                    ...
                    f'db.client_executable_name.alias.callproc.{operation}',
                )
    ...
    def test_patched_execute_calls_timer(self):
        for operation, query in list(self.example_queries.items()):
            with patch('django_statsd.patches.db.statsd') as statsd_mock:
                ...
                self.assertEqual(
                    statsd_mock.timer.call_args[0][0],
                    f'db.client_executable_name.alias.execute.{operation}',
                )
    ...
    def test_patched_executemany_calls_timer(self):
        for operation, query in list(self.example_queries.items()):
            with patch('django_statsd.patches.db.statsd') as statsd_mock:
                ...
                self.assertEqual(
                    statsd_mock.timer.call_args[0][0],
                    f'db.client_executable_name.alias.executemany.{operation}',
                )
